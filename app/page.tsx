...
import { BLUEPRINT_GOALS, siteUrl, SITE_NAME, SITE_TAGLINE } from "@/lib/site";
...
export default async function HomePage() {
  ...
  try {
    ...
    goalSlugs = new Set(published.map((b) => b.goal));
  } catch (err) {
  ...
  const goals = BLUEPRINT_GOALS.filter((g) => goalSlugs.has(g.slug));
      ...
      {goals.length > 0 && (
        <section className="py-14">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <h2 className="font-serif text-2xl font-bold text-ink sm:text-3xl">What do you want AI to help with?</h2>
            <div className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {goals.map((g) => (
                <Link
                  key={g.slug}
                  href={`/blueprints?goal=${g.slug}`}
                  className="group flex items-center justify-between rounded-[2px] border border-rule bg-surface px-5 py-4 transition hover:border-ink/40"
                >
                  <span className="font-semibold text-ink">{g.label}</span>
                  <ArrowRight className="h-4 w-4 text-ink/40 transition group-hover:translate-x-0.5 group-hover:text-accent" />
                </Link>
              ))}
            </div>
          </div>
